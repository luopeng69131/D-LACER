...
Starlink 通过高速运动的低轨卫星为用户终端提供接入服务。卫星运动与切换、链路几何、网络负载和环境条件共同造成显著的短期容量波动。因此，可靠的吞吐量预测是准入控制、自适应流媒体、拥塞控制和带宽预留等网络决策的重要基础。

传统点预测通常通过 MAE 或 RMSE 等对称误差进行优化，但两种误差方向对网络运行的影响并不对称。**高估**可用吞吐量可能导致资源过度准入、丢包和服务质量违约；**低估**则主要造成相对保守的资源分配和利用率损失。因此，安全预测不仅需要保持有效精度，还需要将有害高估出现的频率限制在应用给定的风险预算内。

仅控制平均或边际风险仍不充分。即使整体预算已经满足，违约仍可能集中出现在潜在低容量状态、高预测分歧窗口或较远预测距离上。与此同时，一个 15 步预测只有在完整未来窗口结束后才能获得全部监督信号。这形成了一个**延迟反馈下针对重叠条件风险组的在线控制问题**，也是 D-LACER 的直接研究动机。

<p align="center">
  <img src="assets/research_background.png" width="96%" alt="Starlink 吞吐量预测背景与非对称风险">
</p>

<p align="center"><em>Starlink 的高速移动性带来剧烈吞吐量波动，而预测误差的非对称代价要求采用风险预算约束的安全预测。</em></p>
